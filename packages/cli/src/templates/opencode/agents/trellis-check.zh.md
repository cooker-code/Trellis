...
- **如果不存在标记**：Hook 注入未触发（Windows + Claude Code、`--continue` 恢复、fork 发行版、Hook 被禁用等）。从分派提示第一行 `Active task: <path>` 查找当前 Task 路径（也可运行 `python3 ./.trellis/scripts/task.py current --source` 作为后备方案），然后在工作前依次读取 `<task-path>/check.jsonl`、其中列出的每个文件、`<task-path>/prd.md`、存在时的 `<task-path>/design.md` 和 `<task-path>/implement.md`。
